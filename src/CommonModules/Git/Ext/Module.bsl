...
Процедура ВыполнитьИндексированиеИКоммит( Знач ЛокальныйРепозиторийАдрес, Знач пДанныеВерсии, Знач Хранилище ) Экспорт
	...
	параметрыПользователя = _ПользователиВызовСервера.ПараметрыПользователя( пДанныеВерсии.Пользователь );
	...
	записьТекста.ЗаписатьСтроку( "set GIT_COMMITTER_NAME=" + параметрыПользователя.Имя );
	записьТекста.ЗаписатьСтроку( "set GIT_COMMITTER_EMAIL=" + параметрыПользователя.Email );
