...
Процедура ВыгрузитьВерсииВЛокальныйРепозиторий( Знач Хранилище, КоличествоВерсийВыгружаемыхЗаРаз = 0 ) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	ВерсииКонфигурацийХранилища.Пользователь
	               |ИЗ
	               |	Справочник.ВерсииКонфигурацийХранилища КАК ВерсииКонфигурацийХранилища
	               |ГДЕ
	               |	ВерсииКонфигурацийХранилища.Владелец = &Хранилище
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогИнформация( "Версия: " + НомерВерсииВХранилище + ", " + выборка.Пользователь + ": " + выборка.Комментарий );
			...
			СконфигурироватьПользователя( выборка.Пользователь, Хранилище, Выборка.НомерВерсии );
...
Процедура ВыгрузитьВерсииВУдаленныйРепозиторий( Знач Хранилище ) Экспорт
	...
	Если ЗначениеЗаполнено( выборка.СкриптДляPush ) Тогда
		...
		времКаталог = ЗавершитьРазделителемПутьДоКаталога( выборка.ЛокальныйРепозиторийАдрес );
		времКаталог = времКаталог + ".tmp";
		
		ОбеспечитьКаталог( времКаталог );
		
		скрипт = ЗавершитьРазделителемПутьДоКаталога( времКаталог ) + выборка.ИмяСкрипта;
		
		выборка.ХранилищеДвоичныхДанныхСкрипта.Получить().Записать( скрипт );
		...
		командаЗапуска = СтрШаблон( командаЗапуска,
									скрипт,
									выборка.ЛокальныйРепозиторийАдрес,
									файлВыводаСкрипта);
		
		ВыполнитьКоманду( выборка.ЛокальныйРепозиторийАдрес, командаЗапуска );
