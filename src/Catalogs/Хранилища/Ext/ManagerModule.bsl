...
Процедура ВыгрузитьВерсииВЛокальныйРепозиторий( Знач Хранилище, КоличествоВерсийВыгружаемыхЗаРаз = 0) Экспорт
	...
	Если ЗначениеЗаполнено(КоличествоВерсийВыгружаемыхЗаРаз) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПЕРВЫЕ 1", "ПЕРВЫЕ " + Формат(КоличествоВерсийВыгружаемыхЗаРаз, "ЧГ=0"));	
	Иначе	
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Git.ВыполнитьИндексированиеИКоммит(Хранилище.ЛокальныйРепозиторийАдрес, Выборка.Комментарий, Выборка.ДатаСоздания, Хранилище);
	...
	Исключение
		...
		РегистрыСведений.СтатусыВыгрузки.ЗавершеноСОшибками( Хранилище, ПодробноеПредставлениеОшибки( ИнформацияОбОшибке() ) );
